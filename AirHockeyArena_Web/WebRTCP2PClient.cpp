...
namespace
{
	...
	s3d::String MakeClientOptionsJSON(const Client::ClientOptions& options, const s3d::String& peerID)
	{
		...
		json[U"applicationId"] = options.applicationId;
...
namespace s3d
{
	...
	public:
		...
		{
			if (value.hasElement(U"roomId"))
			{
				if (const auto roomID = value[U"roomId"].getOpt<String>()) destination.id = *roomID;
			}
			else if (value.hasElement(U"id"))
			{
				if (const auto roomID = value[U"id"].getOpt<String>()) destination.id = *roomID;
			}

			destination.listed = true;
			destination.isOpen = true;
			destination.participantCount = 0;
			destination.maxParticipants = 8;
			if (value.hasElement(U"listed"))
				if (const auto listed = value[U"listed"].getOpt<bool>()) destination.listed = *listed;
			if (value.hasElement(U"isOpen"))
				if (const auto isOpen = value[U"isOpen"].getOpt<bool>()) destination.isOpen = *isOpen;
			if (value.hasElement(U"participantCount"))
				if (const auto count = value[U"participantCount"].getOpt<int32>()) destination.participantCount = *count;
			if (value.hasElement(U"maxParticipants"))
				if (const auto maxParticipants = value[U"maxParticipants"].getOpt<int32>()) destination.maxParticipants = *maxParticipants;

			destination.properties.clear();
			const JSON properties = value.hasElement(U"properties")
				? value[U"properties"]
				: (value.hasElement(U"config") ? value[U"config"] : JSON::Parse(U"{}", AllowExceptions::No));
			if (properties.isObject())
			{
				for (const auto& item : properties)
				{
					try
					{
						String propertyValue = item.value.formatMinimum();
						if (const auto stringValue = item.value.getOpt<String>()) propertyValue = *stringValue;
						destination.properties.emplace(ParseInt<uint8>(item.key), propertyValue);
					}
					catch (...) {}
				}
			}
